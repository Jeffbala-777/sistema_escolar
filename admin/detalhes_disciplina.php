...
if ($periodoId === 0 && !empty($periodosDisp)) {
    foreach ($alunosNotas as $aluno) {
        ...
        foreach ($periodosDisp as $p) {
            // Notas por período
            $sqlNP = "SELECT AVG(nota) as media FROM notas WHERE aluno_id = ? AND disciplina_id = ? AND periodo_id = ?";
            $stmtNP = $pdo->prepare($sqlNP);
            $stmtNP->execute([$aluno['id'], $disciplinaId, $p['id']]);
            $resNP = $stmtNP->fetch();
            $notasPorPeriodo[$aluno['id']][$p['id']] = $resNP['media'] !== null ? (float)$resNP['media'] : null;
            
            // Faltas por período
            $sqlFP = "SELECT COUNT(p.id) as total FROM presencas p 
            ...
            $stmtFP->execute([$aluno['id'], $disciplinaId, $p['id'], $p['id']]);
            $resFP = $stmtFP->fetch();
            $faltasPorPeriodo[$aluno['id']][$p['id']] = (int)($resFP['total'] ?? 0);
        }
    }
} else if ($periodoId > 0) {
                            ...
                            <tr>
                                ...
                                <?php if ($periodoId === 0 && !empty($periodosDisp)): ?>
                                    <?php foreach ($periodosDisp as $p): 
                                        ...
                                        $corNota = ($nota !== null && $nota >= $metaCor) ? 'success' : ($nota !== null ? 'danger' : 'secondary');
                                    ?>
                                        <td class="text-center">
                                            <?php if ($nota !== null): ?>
                                                <span class="badge bg-<?= $corNota ?> bg-opacity-10 text-<?= $corNota ?> fs-6 px-2">
